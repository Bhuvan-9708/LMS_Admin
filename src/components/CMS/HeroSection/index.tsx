...
const HeroSection: React.FC = () => {
    ...
    const handleAddSection = () => {
        router.push('/cms/add-hero-section');
    }
    ...
    const handleToggleActive = async (id: string, newStatus: boolean) => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/hero-section/status/${id}`, {
                method: "PATCH",
                    ...
                    <TableContainer component={Paper}>
                        <Table sx={{ minWidth: 650 }} aria-label="herosection table">
                            <TableHead className="bg-primary-50">
                                <TableRow>
                                    {["ID", "Title", "Sub Title", "Description", "Active", "Action"].map((header, index) => (
                                        <TableCell
                                            key={index}
                                            sx={{
                                                fontWeight: "500",
