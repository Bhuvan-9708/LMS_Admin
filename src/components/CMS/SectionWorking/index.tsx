...
const SectionTable: React.FC = () => {
    ...
    const handleAddSection = () => {
        router.push('/cms/add-section-working');
    }
    ...
    const handleToggleActive = async (id: string, newStatus: boolean) => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/section-working/status/${id}`, {
                method: "PATCH",
    ...
    const handleDelete = async (id: string) => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/section-working/${id}`, {
                method: "DELETE",
            });
            if (!response.ok) {
                    ...
                    <TableContainer component={Paper}>
                        <Table sx={{ minWidth: 650 }} aria-label="sections table">
                            <TableHead className="bg-primary-50">
                                <TableRow>
                                    {["ID", "Title", "Sub Title", "Active", "Action"].map((header, index) => (
                                        <TableCell
                                            key={index}
                                            sx={{
                                                fontWeight: "500",
